...
interface DatabaseField {
  name: string
  type: 'SERIAL' | 'VARCHAR' | 'TEXT' | 'NUMERIC' | 'INTEGER' | 'BOOLEAN' | 'DATE' | 'TIMESTAMP' | 'JSONB'
  editable: boolean
  value: any
...
export default function UniversalDatabaseEditor({ companyId, onUpdate }: UniversalDatabaseEditorProps) {
  ...
  const getFieldStyle = (table: string, field: string, isEdited: boolean): string => {
    ...
    switch (fieldType) {
      ...
      case 'JSONB':
        return 'bg-pink-50 hover:bg-pink-100 cursor-pointer border-pink-200'
      default:
                      ...
                      >
                        {isCurrentlyEditing ? (
                          <div className="flex items-center space-x-2">
                            {getFieldType(tableName, field) === 'JSONB' ? (
                              <textarea
                                value={editValue}
